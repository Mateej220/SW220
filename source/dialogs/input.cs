...
namespace SW220
{
	public partial class Dialogs
	{
		...
		public static string Input(int width, string title, string[] message, bool password = false, int limit = 0, Theme? theme = null)
		{
		...
		public static string Input(int width, string title, string[] message, string txt_confirm = "Confirm", bool password = false, int limit = 0, Theme? theme = null)
		{
			// If there is no defined theme, dialog will use default one.					//
			// (New instance of "Theme" [file: source/dialogs/theme.cs] )					//
			theme ??= new Theme();
			...
			Y = Draw.MenuBody(StartX, StartY, Y, width, theme);
			Draw.MenuEnd(StartX, StartY, Y, width, theme);
			Draw.PanelBody(StartX, StartY, Y + 1, width, 0, txt_confirm, theme);
			Draw.PanelEnd(StartX, StartY, Y + 2, width, theme);
